 ...
 *
 * 2010/02/07 some small fixes by Didier Stevens for UNICODE support
 */
...
OutputLastError(LPCTSTR msg)
{
	...
	tmpmsg = (LPTSTR)LocalAlloc(LPTR, _tcslen(msg) + _tcslen(tmp) + 3);
	_stprintf(tmpmsg, _T("%s: %s"), msg, (LPCTSTR)tmp);
	ConOutPrintf(tmpmsg);
	LocalFree((LPVOID)tmpmsg);
	LocalFree(tmp);
}
#endif
...
#ifdef DEBUG_OUTPUT
				OutputLastError(_T("Error protecting memory page"))
#endif
			;
		}
	}
...
#ifdef DEBUG_OUTPUT
				ConOutPrintf(_T("BuildImportTable LoadLibrary: %s\n"), (LPCTSTR)(codeBase + importDesc->Name));
#endif
			HMODULE handle = LoadLibraryA((LPCSTR)(codeBase + importDesc->Name));
#ifdef DEBUG_OUTPUT
				ConOutPrintf(_T("BuildImportTable LoadLibrary handle: 0x%08d\n"), handle);
#endif
...
HMEMORYMODULE MemoryLoadLibrary(const void *data)
{
...
#ifdef DEBUG_OUTPUT
		ConOutPrintf(_T("MemoryLoadLibrary start.\n"));
#endif
	dos_header = (PIMAGE_DOS_HEADER)data;
	if (dos_header->e_magic != IMAGE_DOS_SIGNATURE)
	{
#ifdef DEBUG_OUTPUT
		ConOutPrintf(_T("Not a valid executable file.\n"));
#endif
		return NULL;
	}
	...
	{
#ifdef DEBUG_OUTPUT
		ConOutPrintf(_T("No PE header found.\n"));
#endif
		return NULL;
	}
	...
	{
#ifdef DEBUG_OUTPUT
		ConOutPrintf(_T("BuildImportTable failed.\n"));
#endif
		goto error;
	}
	...
	if (result->headers->OptionalHeader.AddressOfEntryPoint != 0)
	{
		...
		{
#ifdef DEBUG_OUTPUT
			ConOutPrintf(_T("Library has no entry point.\n"));
#endif
			goto error;
		}
		...
		{
#ifdef DEBUG_OUTPUT
			ConOutPrintf(_T("Can't attach library.\n"));
#endif
			goto error;
		}
...
#ifdef DEBUG_OUTPUT
		ConOutPrintf(_T("MemoryLoadLibrary end.\n"));
#endif
	return (HMEMORYMODULE)result;

error:
#ifdef DEBUG_OUTPUT
		ConOutPrintf(_T("MemoryLoadLibrary error.\n"));
#endif
	// cleanup
